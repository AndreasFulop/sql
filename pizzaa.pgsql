...
/* Mi Szundi legtöbbet rendelt pizzája? */
SELECT pnev, vnev, sum(db) FROM pizza
INNER JOIN tetel ON pizza.pazon = tetel.pazon 
INNER JOIN rendeles ON rendeles.razon = tetel.razon
INNER JOIN vevo ON vevo.vazon = rendeles.vazon
WHERE vnev = 'Szundi' GROUP BY pnev, vnev ORDER BY sum(db) desc LIMIT 1;
/* Kik rendeltek pizzát a nyitás napján? */
SELECT vnev FROM vevo
INNER JOIN rendeles ON rendeles.vazon = vevo.vazon
WHERE extract (DAY from idopont) = 1
GROUP BY vnev;
/* Mely pizzák olcsóbbak a Capricciosa pizzánál? */
SELECT pnev FROM pizza
WHERE par < (SELECT par FROM pizza WHERE pnev = 'Capricciosa');
...
SELECT fnev, count(fnev) FROM futar 
INNER JOIN rendeles ON rendeles.fazon = futar.fazon
INNER JOIN tetel ON tetel.razon = rendeles.razon
GROUP BY fnev HAVING count(fnev) >= 10;
/* Mely pizzából fogyott legalább 12 db? */
SELECT pnev, sum(db) FROM pizza
INNER JOIN tetel ON pizza.pazon = tetel.pazon 
GROUP BY pnev HAVING sum(db) > 12;
/*  */
/*  */
/*  */
/*  */
